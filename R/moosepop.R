...
#'
#' @examples
geo.moosepop <- function(column.ana, strat, data, sampled, area, Surveyyear, SurveyName,
                         column.pred, column.lat, column.lon)
{
  ...
  outpt <- list(
    ...
    parmest1 = parmest1,
    parmest2 = parmest2,
    predictions =data.frame(data[,c(column.lat, column.lon, strat, sampled, Surveyyear, SurveyName)],
...
results<-function(data,AA = NULL, metric = "totalmoose"){
  ...
  Surveyyear<-make.names(Surveyyear)
  SurveyName<-make.names(SurveyName)
  sampled<-make.names(sampled)
  column.surveyid<-make.names(column.surveyid)

  STR_calc.out<-tryNULL(moosepop(column.ana = column.ana, strat = strat,data = data, sampled = sampled, area = area,column.pred = column.pred))

  GSPE_calc.out<-tryNULL(geo.moosepop(column.ana = column.ana, strat = strat,data = data,SurveyName = SurveyName,
                                      Surveyyear = Surveyyear,
                                      sampled = sampled, area = area,column.pred = column.pred,
                                      column.lat=column.lat, column.lon=column.lon))
  return(list(STR_calc.out = STR_calc.out,GSPE_calc.out = GSPE_calc.out))
}
...
adult<-AA_abundance(data, column_names, metric = "TotalAdults")
